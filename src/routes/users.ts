...
router.post(
  '/changePassword',
  inputsValidator(changePasswordSchema),
  async (ctx) => {
    const { id, newPassword, oldPassword } = ctx.request.body
    const r = await UserController.changePasswordById(
      id,
      newPassword,
      oldPassword
    )
...
router.post(
  '/changeInfoById',
  inputsValidator(changeUserInfoSchema),
  async (ctx) => {
...
router.get(
  ...
  inputsValidator(followUserSchema),
  async (ctx) => {
...
router.get(
  ...
  inputsValidator(likeArticleSchema),
  async (ctx) => {
...
router.get(
  ...
  inputsValidator(likeCommentSchema),
  async (ctx) => {
    const commentId = ctx.params.commentId
    const userId = ctx.state.user.id
    ctx.body = await UserController.likeCommentById(userId, commentId)
  }
)

router.get(
  '/collectArticle/:articleId',
  inputsValidator(collectArticleSchema),
  async (ctx) => {
    const articleId = ctx.params.articleId
    const userId = ctx.state.user.id
