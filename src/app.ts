...
const sessionConfig = {
  ...
  overwrite:
    true /** (boolean)如果是 true, 在同一个请求中设置相同名称的所有 Cookie（无论路径或域）是否在设置此Cookie 时从 Set-Cookie 消息头中过滤掉. (默认 true) */,
  httpOnly:
    true /** (boolean) 表示 cookie 是否仅通过 HTTP(S) 发送，, 且不提供给客户端 JavaScript (default true) */,
  signed: true /** (boolean) 是否要对 cookie 进行签名 (默认 true) */,
  rolling:
    false /** (boolean) 强制在每个响应上设置会话标识符 cookie。 过期重置为原来的maxAge，重置过期倒计时。 (默认 is false) */,
  renew:
    false /** (boolean) 当会话即将过期时更新会话，因此我们可以始终保持用户登录。 (默认 is false)*/,
  secure: false /** (boolean) secure cookie*/,
  sameSite:
    false /** (string) 表示该 cookie 是否为 "相同站点" cookie (默认为 false).  */,
}
