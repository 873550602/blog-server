...
export default class UserDio {
   ...
   * 通过id获取用户
   */
  static async getUserInfoById(id: string, fields: string[] | string = "*"): Promise<User | undefined> {
    if (Array.isArray(fields)) fields = fields.join(',')
    const r = await Mysql.db.select(fields).from(TableName.user).where('id', id).queryRow()
    return deSerializeSqlData(r, ['labes', 'likes', 'collects', 'followeds', 'follows'])
  }
  /**
   * 通过id获取已关注的用户
   */
   ...
   * 登录认证
   */
  static async login(account: string, password: string): Promise<User | undefined> {
    const r = await Mysql.db.select("*").from(TableName.user).where('account', account).where('password', password).queryRow()
   ...
   * 通过id修改密码
   */
  static async changePasswordById(id: string, password: string): Promise<boolean> {
